...
```
*/
inline constexpr auto dot(const std::array<int64_t, 3>& a, const std::array<int64_t, 3>& b) -> int64_t {
...
inline constexpr auto cross(const std::array<int64_t, 3>& a,
                            const std::array<int64_t, 3>& b) -> std::array<int64_t, 3> {
    return {
        a[1] * b[2] - a[2] * b[1],
        a[2] * b[0] - a[0] * b[2],
        a[0] * b[1] - a[1] * b[0],
...
plckr(const int64_t& ld, const std::array<int64_t, 3>& p,
      const int64_t& mu, const std::array<int64_t, 3>& q) -> std::array<int64_t, 3> {
    return {
        ld * p[0] + mu * q[0],
        ld * p[1] + mu * q[1],
        ld * p[2] + mu * q[2],
